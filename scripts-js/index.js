...
const downloadRandomUser = (params) => {
    ...
    return new Promise((res, rej) => {
        ...
        conn.onload = (e) => {
            ...
            if (doesntExist(data) || doesntExist((_a = data === null || data === void 0 ? void 0 : data.results) === null || _a === void 0 ? void 0 : _a[0])) {
                return rej({ msg: "noData", info: data === null || data === void 0 ? void 0 : data.info });
            }
            res(data.results);
        };
        conn.onerror = e => {
 ...
 */
const countRefresh = () => {
    let numberOfRefreshes = Number(sessionStorage.getItem("refreshCounter"));
    if (doesntExist(numberOfRefreshes))
        sessionStorage.setItem("refreshCounter", "1");
    else
        sessionStorage.setItem("refreshCounter", `${numberOfRefreshes + 1}`);
...
window.onload = () => {
    try {
        onRefresh();
        let canvas = document.querySelector('.data_chart');
